...
const FEE = 500;
const MAX_RETRIES = 1;
// Generate a random number between 1 and 10 for swap rounds
function getRandomSwapRounds() {
...
async function main() {
    const inviteCode = 'Ocix9GuyM816tIUD';
    try {
        ...
        for (let i = 0; i < walletData.length; i++) {
            ...
            try {
                ...
                const swapRounds = getRandomSwapRounds();
                log(`Using random swap rounds: ${swapRounds}`, "info");
                const swapSuccess = await performMultipleSwaps(provider, wallet, swapRounds);
                if (swapSuccess) {
                    swapSuccessCount++;
                    await printBalances(provider, wallet);
